...
class WMIObject final : CComPtr<IWbemClassObject> {
public:
	using CComPtr<IWbemClassObject>::CComPtr;

	std::vector<CComBSTR> GetNames() const;
};

struct WMIHelper abstract final {
	static HRESULT Init(PCWSTR computerName, PCWSTR ns, IWbemServices** ppWmi);
	static std::wstring GetStringProperty(IWbemClassObject* pObj, PCWSTR name);
	static std::vector<WMIObject> EnumNamespaces(IWbemServices* pWmi);
	static std::vector<WMIObject> EnumClasses(IWbemServices* pSvc, bool deep, bool includeSystemClasses = false);
	static std::vector<WMIObject> EnumInstances(PCWSTR name, IWbemServices* pSvc, bool deep);
	...
	static std::vector<CComBSTR> GetNames(IWbemQualifierSet* pObj);
};
