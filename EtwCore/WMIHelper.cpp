...
std::vector<WMIObject> WMIHelper::EnumNamespaces(IWbemServices* pWmi) {
	...
	while (S_OK == spEnum->Next(WBEM_INFINITE, 1, &spObj, &count)) {
		ns.emplace_back(spObj);
		spObj.Release();
	}
	return ns;
}

std::vector<WMIObject> WMIHelper::EnumClasses(IWbemServices* pSvc, bool deep, bool includeSystemClasses) {
	std::vector<WMIObject> classes;
	CComPtr<IEnumWbemClassObject> spEnum;
	...
	while (S_OK == spEnum->Next(WBEM_INFINITE, 1, &spObj, &count)) {
		if (!includeSystemClasses) {
			...
			if (_wcsicmp(dynasty.c_str(), L"__SystemClass") == 0) {
				spObj.Release();
				continue;
			}
		}
		classes.emplace_back(spObj);
		spObj.Release();
	}
	return classes;
}

std::vector<WMIObject> WMIHelper::EnumInstances(PCWSTR name, IWbemServices* pSvc, bool deep) {
	std::vector<WMIObject> instances;
	CComPtr<IEnumWbemClassObject> spEnum;
...
std::vector<CComBSTR> WMIHelper::GetNames(IWbemClassObject* pObj) {
	...
	SAFEARRAY* sa;
	if (FAILED(pObj->GetNames(nullptr, 0, nullptr, &sa)))
		return {};
...
std::vector<CComBSTR> WMIHelper::GetNames(IWbemQualifierSet* pObj) {
	std::vector<CComBSTR> names;
	SAFEARRAY* sa;
	if (FAILED(pObj->GetNames(0, &sa)))
		return {};
...
std::wstring WMIHelper::GetStringProperty(IWbemClassObject* pObj, PCWSTR name) {
	CComVariant value;
...
std::vector<CComBSTR> WMIObject::GetNames() const {
	return WMIHelper::GetNames(p);
}
